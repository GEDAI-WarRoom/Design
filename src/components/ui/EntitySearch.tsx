...
import {
	...
	FieldTooltip,
	SimNao,
} from "../../components/ui/FormKit"; // Centralizado em uma única importação
					...
					{items.map((item, index) => (
						<div
							...
							{showCounter &&
								...
								(smallCounter ? (
									<div className="w-5 h-5 rounded-full bg-[#1A7A3C] text-white flex items-center justify-center flex-shrink-0 font-sans select-none">
			...
			{!disabled && (
				<div className="w-full flex justify-start">
					<button
						type="button"
						onClick={onAddItem}
						className="mt-2 flex items-center gap-2 text-sm font-semibold px-4 py-2.5 rounded-md bg-[#1A7A3C] text-white hover:bg-[#15612F] transition cursor-pointer">
						<PlusCircle size={16} />
						{addButtonLabel}
					</button>
				</div>
...
// MODAL EXPLORAÇÃO
export function ExploracaoPecuariaInput({
	value,
	onChange,
	onEyeClick,
	required = false,
...
export function EstabelecimentoAgropecuarioInput({
	value,
	onChange,
	onEyeClick,
	required = false,
...
export function DoencaInput({
	...
	const BlacklistedOuSelecionada = dadosFiltrados.find(
		(x: any) => x.nome === value || x.codigo === value,
	);
			...
			<div className="flex items-end gap-2 w-full">
				...
				<div className="flex-1 w-full">
					<EntitySearchInput
						...
						subtitle={
							apenasVacinaveis
								? "Busque por uma doença cadastrada:"
								: "Busque por uma doença cadastrada:"
						}
						icon={
							<img
...
// COMPONENTE: FornecedorInput
// ==========================================================
export function DestinatarioInput({
	value,
	onChange,
	onEyeClick,
	required = false,
...
const DISTRITOS_MOCK = [
	"Abadia dos Dourados",
	"Abaeté",
	"Distrito de Campos do Meio",
	"Vale do Norte",
];
			...
			<div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl p-8 flex flex-col gap-6 relative font-sans text-gray-800 animate-in fade-in zoom-in-95 duration-150">
				...
				<div className="w-full h-[240px] rounded-xl border border-gray-200 overflow-hidden relative bg-gray-100 flex items-center justify-center">
					...
					<div className="relative z-20 flex flex-col items-center">
						<MapPin
							size={34}
							className="text-[#1A7A3C] fill-[#1A7A3C] drop-shadow-md animate-bounce"
						/>
					</div>
...
export function BlocoEnderecoFields({
	...
	return (
		<div className="flex flex-col gap-4 border-t border-gray-100 pt-4 first:border-0 first:pt-0">
			...
			<div
				className={`grid grid-cols-1 gap-3 ${isRural ? "md:grid-cols-3" : "md:grid-cols-5"}`}>
				<FloatSelect
					label="Zona"
					required
					value={data.zona}
			...
			<div className="grid grid-cols-1 md:grid-cols-12 gap-3">
				<div
					className={`relative hover:z-30 focus-within:z-30 ${isRural ? "md:col-span-12" : "md:col-span-7"}`}>
					<FloatInput
						label="Endereço"
						required
						value={data.endereco}
...
interface BlocoContatoFieldsProps {
	...
	proprietariosDisponiveis: Array<{
		id: string;
		nome: string;
		cpf: string;
		email?: string;
...
export function BlocoContatoFields({
	...
	const removerProprietario = (id: string) => {
		handleFieldChange(
			...
			data.proprietariosSelecionados.filter(
		...
		<div className="flex flex-col gap-5">
			...
			<div className="flex flex-col gap-2">
				<label className="text-xs font-semibold text-gray-600 block">
					Utilizar Contato de Proprietários?{" "}
			...
			{data.utilizarContatoProprietario === "Sim" && (
				<div className="flex flex-col gap-4 border-t border-gray-100 pt-4 animate-in fade-in duration-200">
					...
					<MultiSearchModal
						...
						columns={[{ label: "Nome do Proprietário", key: "nome" }]}
					...
					<div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
						...
						<div className="md:col-span-4">
							<FloatInput
								...
								value={data.emailFixo || ""}
								onChange={(v) => handleFieldChange("emailFixo", v)}
							/>
						</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
						...
						<div className="md:col-span-4">
							<FloatInput
								...
								value={data.telefoneFixo || ""}
								onChange={(v) =>
									handleFieldChange("telefoneFixo", aplicarMascaraTelefone(v))
								}
								maxLength={15}
							/>
						</div>
						<div className="md:col-span-6">
							<div className="relative border border-gray-300 rounded-md p-3 bg-white">
								...
								/>
								<div className="text-right text-xs text-gray-400 mt-1">
									{(data.telefoneFixoObs || "").length}/1500
								</div>
							</div>
						</div>
					</div>
			...
			<div className="border-t border-gray-100 pt-4 mt-2">
				...
				<DynamicListWrapper
					...
					onAddItem={() => {
						...
						handleFieldChange("contatosAdicionais", [
							...(data.contatosAdicionais || []),
							novo,
						]);
					}}
					...
					{(cont: ContatoAdicional, index: number) => (
						<div
							key={cont.id}
							className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start relative animate-in slide-in-from-top-2 duration-150 w-full mb-4">
							<div className="md:col-span-2">
								<FloatSelect
									label="Tipo"
									required
								...
								<div className="md:col-span-4">
									<FloatInput
										...
										onChange={(v) => {
											const list = data.contatosAdicionais.map((c, i) =>
												i === index
													? { ...c, telefone: aplicarMascaraTelefone(v) }
													: c,
											);
											handleFieldChange("contatosAdicionais", list);
										}}
								...
								<div className="relative border border-gray-300 rounded-md p-3 bg-white">
									...
									<textarea
										...
										onChange={(e) => {
											const list = data.contatosAdicionais.map((c, i) =>
												i === index
													? { ...c, observacao: e.target.value.slice(0, 1500) }
													: c,
											);
											handleFieldChange("contatosAdicionais", list);
										}}
...
export function NucleoInput({
	...
	const entidadeSelecionada = NUCLEOS_MOCK.find(
		(x) => x.nome === value || x.codigo === value,
	);
...
export const VacinadorBruceloseInput: React.FC<
	VacinadorBruceloseInputProps
> = ({
	value,
	onChange,
	onEyeClick,
...
export const ProfissionalAnimalInput: React.FC<
	ProfissionalAreaAnimalInputProps
> = ({
	value,
	onChange,
	onEyeClick,
...
export function UnidadeAdministrativaInput({
	value,
	onChange,
	onEyeClick,
	required = false,
...
export const ResponsavelTecnicoInput: React.FC<
	ResponsavelTecnicoInputProps
> = ({
	value,
	onChange,
	onEyeClick,
...
export const CERTIFICADORAS_INPUT_MOCK = [
	{
		id: 1,
		nome: "Rastro de Boi",
		proprietario: "72.375.545/0001-93 - Rastro de Boi Certificação",
		responsavel: "555.009.956-40 - Gustavo de Souza Sobrinho",
		status: "Regular",
		municipio: "Lavras",
		uf: "MG",
		estado: "Minas Gerais",
	...
	{
		id: 2,
		nome: "Certificadora Condão",
		proprietario: "45.221.118/0001-40 - Rastreabilidade Sul Ltda.",
		responsavel: "333.221.115-09 - Marina Couto Dias",
		status: "Bloqueada",
		municipio: "Uberaba",
		uf: "MG",
		estado: "Minas Gerais",
	...
	{
		id: 3,
		nome: "Boi Certo Rastreamento",
		proprietario: "72.375.545/0001-93 - Rastro de Boi Certificação",
		responsavel: "555.009.956-40 - Gustavo de Souza Sobrinho",
		status: "Acesso regular via liminar",
		municipio: "Barretos",
		uf: "SP",
		estado: "São Paulo",
		situacao: "Ativo",
