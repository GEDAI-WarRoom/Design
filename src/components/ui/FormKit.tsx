...
export function FieldTooltip({ text }: { text?: string }) {
	...
	return (
		...
		<div className="cursor-pointer text-gray-400 hover:text-gray-600 transition flex items-center relative group pointer-events-auto">
			<Info size={13} />
			<div className="absolute left-6 bottom-1 bg-[#e0e0e0] border border-gray-300 text-black text-[11px] py-1.5 px-3 rounded shadow-md z-[9999] pointer-events-none hidden group-hover:flex items-start gap-1.5 w-72 max-w-xs normal-case font-normal leading-relaxed">
				{" "}
				<Info size={12} className="text-gray-600 flex-shrink-0" />
				<span className="text-xs text-black whitespace-pre-line">{text}</span>
			</div>
		</div>
	);
}
	...
	return (
        ...
        ${(onClick || isCalendar) && !disabled ? "cursor-pointer" : ""} ${className}`}>
          ...
          ${active ? "top-1 text-[10px] text-gray-400 font-medium" : "top-1/2 -translate-y-1/2 text-sm text-gray-400"}`}>
			...
			{icon && (
				<div
					className={`absolute left-3 bottom-2.5 ${disabled ? "text-gray-300" : "text-[#1A7A3C]"}`}>
					{icon}
				</div>
			)}
...
export function CustomButton({
	...
	const baseStyle =
		"h-11 px-6 font-semibold text-sm rounded-lg transition shadow-sm flex items-center justify-center gap-2 select-none disabled:opacity-50 disabled:cursor-not-allowed";
	const variants = {
		filled: "bg-[#1A7A3C] hover:bg-[#15612F] text-white",
		outlined:
			"bg-white border border-[#1A7A3C] text-[#1A7A3C] hover:bg-green-50/40",
	};
	return (
		<button
			type="button"
...
export function FloatSelect({
	...
	return (
        ...
        ${isOpen && !disabled ? "border-[#1A7A3C] ring-1 ring-[#1A7A3C] z-30" : "z-10"}`}>
			<div
				className={`absolute left-3 transition-all duration-150 pointer-events-none select-none flex items-center gap-1.5 
        ${active ? "top-1 text-[10px] text-gray-400 font-medium" : "top-1/2 -translate-y-1/2 text-sm text-gray-400"}`}>
				<span>
					{label}
					{required && <span className="text-red-500 ml-0.5">*</span>}
			...
			<div className="w-full flex items-center h-6 text-sm text-gray-800">
				<span className={`flex-1 truncate ${disabled ? "text-gray-500" : ""}`}>
					{selectedLabel}
				</span>
				{!disabled && (
					<ChevronDown
						size={16}
						className={`text-gray-400 flex-shrink-0 transition-transform mb-2 ${isOpen ? "rotate-180" : ""}`}
					/>
				)}
			</div>
			{isOpen && !disabled && (
        ...
        ${active ? "top-1 text-[10px] text-gray-400 font-medium" : "top-1/2 -translate-y-1/2 text-sm text-gray-400"}`}>
			...
			<div className="w-full flex items-center h-6 text-sm text-gray-800">
				...
				<ChevronDown
					size={16}
					onClick={() => !disabled && setIsOpen(!isOpen)}
					className={`text-gray-400 flex-shrink-0 cursor-pointer transition-transform ${isOpen ? "rotate-180" : ""}`}
				/>
			</div>
...
export function CustomRadio({
	...
	return (
		<label
			className={`flex items-center gap-2 text-sm text-gray-700 font-medium select-none group ${disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}>
	...
	return (
		<div className="w-[340px] flex flex-col gap-1">
          ...
          ${disabled ? "bg-gray-50 border-gray-200 cursor-not-allowed" : "bg-white border-gray-300 cursor-pointer hover:border-[#1A7A3C]"}`}>
				<Paperclip
					size={18}
					className={disabled ? "text-gray-300" : "text-[#1A7A3C]"}
				/>
			...
			{subtitle && (
				<span className="text-[11px] text-gray-400 pl-1">{subtitle}</span>
			)}
		</div>
	);
}
...
export function SearchModal<T extends { id: string | number }>({
	...
	const totalPaginas = Math.max(
		1,
		Math.ceil(resultadosOrdenados.length / PAGE_SIZE),
	);
	const paginaAtual = Math.min(pagina, totalPaginas);
	const inicio =
		resultadosOrdenados.length === 0 ? 0 : (paginaAtual - 1) * PAGE_SIZE + 1;
	const fim = Math.min(paginaAtual * PAGE_SIZE, resultadosOrdenados.length);
	const itensPagina = resultadosOrdenados.slice(
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
			<div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-8 flex flex-col gap-6 relative">
				...
				<div className="flex flex-row items-end gap-3 w-full relative">
					...
					{headerActions && (
						<div className="w-[180px] flex-shrink-0 z-20">{headerActions}</div>
					)}
							...
							<>
											...
											{columns.map((col) => (
												<th
													...
													<div className="flex items-center gap-1">
														{col.label}
														<span
															className={`transition-opacity text-gray-400 ${sortKey === col.key ? "opacity-100" : "opacity-30"}`}>
															{sortKey === col.key ? (
																sortDir === "desc" ? (
																	<ChevronUp size={16} />
																) : (
												...
												<tr
													...
													<td className="py-3 pr-2">
														<div className="flex items-center justify-center">
															<div
																className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? "border-[#1A7A3C]" : "border-gray-300"}`}>
																{isSelected && (
													...
													{columns.map((col) => (
														<td
															key={String(col.key)}
															className={`py-3 text-sm whitespace-pre-line ${isSelected ? "text-[#1A7A3C] font-medium" : "text-gray-700"}`}>
															{col.render
								...
								<div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-100">
									<span className="text-xs text-gray-500">
										Itens por página: {PAGE_SIZE}
									</span>
									<div className="flex items-center gap-3 text-xs text-gray-500">
										<span>
											Mostrando de {inicio} a {fim} de{" "}
											{resultadosOrdenados.length} resultado
										...
										{itensPagina.map((item) => {
											...
											return (
												<tr
													...
													{columns.map((col, index) => (
														<td
															key={`cell-${item.id}-${String(col.key)}-${index}`}
															className={`py-3 text-sm ${isChecked ? "text-[#1A7A3C] font-medium" : "text-gray-700"}`}>
															{col.render
																? col.render(item[col.key], item)
																: String(item[col.key] ?? "")}
														</td>
			...
			<div
				...
				{options.map((option, index) => {
					...
					return (
						<div
							key={currentId}
							className={`flex items-start gap-1.5 ${disabled ? "opacity-60" : ""} ${
								orientation === "grid" ? "w-full" : "w-fit"
							}`}>
							{/* O label mantém a estrutura flex original do grupo */}
							<label
								className={`inline-flex items-center gap-3 group w-full ${disabled ? "cursor-not-allowed" : "cursor-pointer"}`}>
	...
	return (
		<div className="flex flex-col gap-3 font-sans select-none w-full">
			...
			<div className="flex items-center gap-6 h-6">
				<label
					className={`flex items-center gap-2 text-sm text-gray-700 font-medium ${disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer group"}`}>
					<div className="relative flex items-center justify-center">
						<input
							type="radio"
							name={name}
						...
						/>
						<div
							className={`w-4 h-4 rounded-full border transition-all flex items-center justify-center ${isSim ? "border-[#1A7A3C] bg-white" : "border-gray-300 bg-white group-hover:border-gray-400"}`}>
				...
				<label
					...
					<div className="relative flex items-center justify-center">
						...
						/>
						<div
							className={`w-4 h-4 rounded-full border transition-all flex items-center justify-center ${isNao ? "border-[#1A7A3C] bg-white" : "border-gray-300 bg-white group-hover:border-gray-400"}`}>
					...
					{options.map((option) => {
						...
						return (
							<label
								...
								<span
									className={`text-sm leading-normal break-words pr-2 transition-colors ${
					...
					<div className="flex flex-wrap items-center justify-between pt-4 w-full text-sm text-gray-500 px-2 ">
						...
						<div className="flex gap-6 items-center">
							<div className="flex gap-2 items-center">
								...
								<button
									type="button"
									className="text-[#008446] disabled:opacity-40 cursor-pointer">
									<ChevronLeft size={18} />
								</button>
								<button
									type="button"
							...
							{variant === "com-vinculacao" && (
								<button
									type="button"
									className="flex gap-1.5 items-center text-[#008446] font-medium hover:underline cursor-pointer">
			...
			<div className="h-[60px] w-full flex items-center justify-between px-[12px]">
				<div className="flex gap-[12px] items-center min-w-0 w-full">
					...
					<div className="flex flex-col gap-[2px] items-start min-w-0 flex-1">
						<span className="text-[13px] font-semibold text-[#1d1d1f] truncate w-full">
							{label}
						</span>
						{exibirDetalhes && (
							<span className="text-[10px] font-normal text-gray-500 truncate w-full">
								{subLabel}
							</span>
						)}
					</div>
			...
			<div className="flex-1 flex flex-col pt-[24px] pb-[8px] px-[20px] gap-[10px]">
				...
				<div className="flex gap-[12px] items-start pb-[8px] px-[5px] border-b border-gray-100">
					...
					<div className="flex flex-col items-start min-w-0 flex-1">
						...
						<span className="text-[11px] font-normal text-gray-500 truncate w-full">
							{subtitlePrimary}
						</span>
						{captionPrimary && (
			...
			<div className="flex items-center justify-end pb-[14px] px-[25px] w-full bg-white z-10">
				<div className="flex gap-[10px] items-center">
					...
					<button
						...
						<svg
							className="absolute block inset-0 size-full p-0.5"
							fill="none"
							preserveAspectRatio="none"
							viewBox="0 0 24 24">
...
export function EvaluationActiveCard({
	...
	return (
		<div className="w-full bg-white border border-gray-200 shadow-sm flex flex-col relative rounded-xl overflow-hidden">
			{" "}
			<div
			...
			<div className="flex justify-between items-center px-5 pt-4 pb-2">
				<span className="text-xs font-semibold text-gray-700">
					Atualizado em:{" "}
					<span className="font-normal text-gray-600">{updatedAt}</span>
				</span>
			...
			<div
				...
				<div className="flex gap-[12px] items-center justify-center w-full">
					<div className="flex min-h-[40px] items-center justify-center gap-2">
						{" "}
						{/* Adicionado gap-2 para espaçamento */}
						{/* ✨ PASSO 2: Renderizar o ícone caso ele exista */}
						{icon && (
							<div className="flex items-center text-[#1d1d1f]">{icon}</div>
						)}
						<span
							style={{
								...LAT,
				...
				<div className="flex gap-[12px] items-center justify-center pb-[10px] pt-[40px] w-full">
					...
					<button
						...
						<span
							style={{ ...LAT, fontSize: 15, fontWeight: 700, color: "white" }}>
							{saveLabel}
						</span>
					</button>
				</div>
