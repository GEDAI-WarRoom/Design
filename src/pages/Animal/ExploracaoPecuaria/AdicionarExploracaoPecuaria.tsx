...
const TIPOS_PRODUTOR = [
  ...
  "Assentado",
  "Comodatário",
  "Beneficiário de doação com reserva de usufruto",
  "Meeiro",
  "Parceiro Rural",
  "Posseiro",
  "Possuidor",
...
const EQUI_TIPO_EXPLORACAO = [
  "Propriedade de Espera de Abate de Equídeos (PEAE)",
  "Haras",
  "Alojamento",
  "Unidade Militar",
  "Hospital Veterinário",
  "Centro de Ensino e Pesquisa",
  "Propriedades Fornecedoras de Equídeos (PFE)",
  "Central de Reprodução",
  "Esporte",
  "Lazer",
  "Equoterapia",
  "Propriedade Rural Comum (Animais de Trabalho)",
];
const EQUI_INSTAL = [
  "Curral de Manejo",
  "Redondel",
...
const ORIGEM_CAPTACAO = [
  "Dentro do Estabelecimento",
  "Fora do Estabelecimento",
];
const FONTE_CAPTACAO = [
...
const FINALIDADE_PRODUCAO = [
  "Ciclo Completo",
  "Reprodução/Larvicultura",
  "Cria/Recria",
  "Engorda",
  "Recreação",
  "Quarentena",
...
const TIPO_PISC_ORNAMENTAL = [
  "Unidade de Distribuição",
  "Unidade de Produção",
  "Unidade de Produção de Pequeno Porte",
];
...
const SIST_SEMIFECHADO_ORNAMENTAL = [
  "Tanque Escavado",
  "Tanque Suspenso",
  "Raceway",
];
...
const ABASTECIMENTO = [
  ...
  "Passa de um Tanque para Outro",
  "Não se aplica",
];
const LOCAL_DESCARTE = [
  "Mesmo Corpo de Captação",
  "Outro Corpo d'água",
  "Rede de Esgoto",
  "Outra Unidade de Criação",
];
const DESTINO_ORNAMENTAL = [
  "Unidade de Produção",
  "Revendedora",
  "Distribuidor",
  "Consumidor Final",
];
const DESTINO_DEMAIS = [
  "Estabelecimento com inspeção SIF",
  "Estabelecimento com inspeção SIE",
  "Estabelecimento com inspeção SIM",
  "Consumidor Final",
  "Peixaria",
  "Outros Estabelecimentos de Aquicultura",
  "Outro",
];
...
const ESTABELECIMENTOS_MOCK = [
  {
    ...
    municipio: "Lavras - MG",
    areaProdutivaHa: 1000,
    areaProdutivaM2: 10000000,
  },
  {
    id: 2,
    codigo: "42001040005",
    ...
    <SubGrupo titulo={titulo} comDivisor>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
        <FloatInput
          ...
          onChange={(v) => setCiclo({ ...ciclo, quantidade: v })}
          maxLength={255}
        />
        {unidadeReadonly ? (
          <FloatInput
            ...
            required
            disabled
            value={ciclo.unidade || unidadeOpcoes[0]}
            onChange={() => {}}
          />
        ) : (
          <FloatSelect
            ...
            required
            value={ciclo.unidade}
            onChange={(v) => setCiclo({ ...ciclo, unidade: v })}
            options={toOptions(unidadeOpcoes)}
          />
        )}
        <FloatInput
          ...
          required
          value={ciclo.tamanho}
          onChange={(v) => setCiclo({ ...ciclo, tamanho: v })}
          maxLength={255}
        />
        <FloatInput
          ...
          onChange={(v) => setCiclo({ ...ciclo, quantidadeTanques: v })}
          maxLength={255}
        />
      </div>
    </SubGrupo>
...
export function AdicionarExploracaoPecuariaPage({
  ...
  const areaProdutiva = estabelecimento
    ? unidadeArea === "Hectares"
      ? estabelecimento.areaProdutivaHa
      : estabelecimento.areaProdutivaM2
    : null;
  const areaUtilNum = Number(String(areaUtil).replace(",", ".")) || 0;
  const sobreposicao =
    areaProdutiva != null && areaUtilNum > Number(areaProdutiva);
  ...
  const bovInstalOpcoes = bovArea.includes("Pecuária de Leite")
    ? BOV_INSTAL_LEITE
    : BOV_INSTAL_OUTRO;
  ...
  const showRepro =
    ...
    finalidadeProducao === "Reprodução/Larvicultura";
  const showEngorda =
    finalidadeProducao === "Ciclo Completo" || finalidadeProducao === "Engorda";
  const showCria =
    finalidadeProducao === "Ciclo Completo" ||
    finalidadeProducao === "Cria/Recria";
  ...
  const sistemaProducaoOpcoes = isOrnamental
    ...
    : SISTEMA_PRODUCAO;
  const sistSemifechadoOpcoes = isOrnamental
    ? SIST_SEMIFECHADO_ORNAMENTAL
    : SIST_SEMIFECHADO;
              ...
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                  <FloatSelect
                    ...
                    options={toOptions(UNIDADE_AREA)}
                  />
                  <FloatInput
                    label="Área Produtiva do Estabelecimento"
                    required
                    disabled
                    value={areaProdutiva != null ? String(areaProdutiva) : ""}
                    onChange={() => {}}
                  />
                  <FloatInput
                    label={
                      <div className="flex items-center gap-1.5 pointer-events-auto group relative">
                        ...
                        <div className="absolute left-0 top-[34px] pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-[#e0e0e0] text-black text-xs rounded-md p-2 shadow-lg w-64 z-50 font-normal normal-case">
                          A área útil da exploração deve respeitar os limites da
                          área produtiva disponível para a abertura de
                          explorações no estabelecimento agropecuário. É
                          possível que o estabelecimento já possua outras
                          explorações abertas (pecuárias e agrícolas), o que
                          pode levar à sobreposição.
                        </div>
                      </div>
                    }
          ...
          <div className="flex flex-col gap-6">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-4 items-center">
                ...
                <ProdutorInput
                  ...
                  onEyeClick={() => {
                    if (produtor?.codigo)
                      alert(`Visualizar detalhes: ${produtor.codigo}`);
                    else
                      alert(
                        "Por favor, digite ou selecione uma revendedora primeiro.",
                      );
            ...
            <SubGrupo titulo="Outros Produtores" comDivisor>
              <DynamicListWrapper
                ...
                onAddItem={() =>
                  setOutrosProdutores((p) => [
                    ...p,
                    { uid: uid("pr"), tipo: "", produtor: null },
                  ])
                }
                onRemoveItem={(i: number) =>
                  setOutrosProdutores((p) => p.filter((_, idx) => idx !== i))
                }
              >
                {(item: any) => (
                    ...
                    <ProdutorInput
                      ...
                      onChange={(entidadeSelecionada) =>
                        setOutrosProdutores((p) =>
                          p.map((x) =>
                            x.uid === item.uid
                              ? { ...x, produtor: entidadeSelecionada }
                              : x,
                          ),
                        )
                      }
                      onEyeClick={() => {
                        if (item.produtor?.codigo)
                          alert(`Visualizar detalhes: ${item.produtor.codigo}`);
                        else
              ...
              <SubGrupo titulo="Contrato de Vínculo" comDivisor>
                ...
                <div className="flex gap-3 items-start w-full mb-4">
                  ...
                  <UploadField
                    ...
                    onSelectFile={() =>
                      setContratoVinculo("contrato_de_vinculo.pdf")
                    }
                  />
                    ...
                    <>
                      ...
                      <div className="h-12 flex items-center gap-1">
                        <button
                          type="button"
                          onClick={() =>
                            alert(`Fazendo download de: ${contratoVinculo}`)
                          }
                  ...
                  <SubGrupo titulo="Raça" comDivisor>
                    <DynamicListWrapper
                      ...
                      onAddItem={() =>
                        setSubespecies((p) => [
                          ...p,
                          { uid: uid("sub"), sub: null },
                        ])
                      }
                      onRemoveItem={(i: number) =>
                        setSubespecies((p) => p.filter((_, idx) => idx !== i))
                      }
                    >
                      {(item: any) => (
                        <EntitySearchInput
                ...
                <div className="flex flex-col gap-6 mt-2">
                  ...
                  {isCapOv && (
                    <>
                      <CheckboxGroup
                        ...
                        required
                        actionLabel=""
                        options={toCheck(CAP_AREA)}
                        defaultValue={capArea}
                        onChange={setCapArea}
                        orientation="horizontal"
                      />
                      <CheckboxGroup
                        ...
                        required
                        actionLabel=""
                        options={toCheck(CAP_INSTAL)}
                        defaultValue={capInstal}
                        onChange={setCapInstal}
                        orientation="horizontal"
                      />
                    </>
                  )}
                    ...
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                        <FloatSelect
                          ...
                          required
                          value={equiArea}
                          onChange={setEquiArea}
                          options={toOptions(EQUI_AREA)}
                        />
                        <FloatSelect
                          label="Sistema de Criação"
                          required
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
                        <CheckboxGroup
                          ...
                          required
                          actionLabel=""
                          options={toCheck(EQUI_TIPO_EXPLORACAO)}
                          defaultValue={equiTipoExp}
                          onChange={setEquiTipoExp}
                        ...
                        <CheckboxGroup
                          ...
                          required
                          actionLabel=""
                          options={toCheck(EQUI_INSTAL)}
                          defaultValue={equiInstal}
                          onChange={setEquiInstal}
                          orientation="vertical"
                        />
                      </div>
                    </>
                  ...
                  {isPeixes &&
                    (() => {
                      ...
                      const aptidaoSomenteLeitura =
                        nomeLC.includes("redondo") ||
                        nomeLC.includes("ornamental");
                      ...
                      return (
                        <>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {fonteCaptacao.includes("Reservatório") && (
                              <FloatInput
                                label="Nome do Reservatório"
                                required
                                value={nomeReservatorio}
                                onChange={setNomeReservatorio}
                                maxLength={255}
                              />
                            )}
                          </div>
              ...
              <div className="flex flex-col gap-12 mt-2">
                ...
                <div className="grid grid-cols-3 md:grid-cols- gap-4 items-start">
                  ...
                  <CheckboxGroup
                    ...
                    required
                    actionLabel=""
                    options={toCheck(LOCAL_DESCARTE)}
              ...
              <SubGrupo titulo="Destino dos Animais" comDivisor>
                <div className="flex flex-col gap-6 mt-2">
                  <CheckboxGroup
                    ...
                    required
                    actionLabel=""
                    options={toCheck(destinoOpcoes)}
                    defaultValue={tipoDestino}
                    onChange={setTipoDestino}
                    orientation="horizontal"
                  />
                  {tipoDestino.includes("Outro") && (
                    <FloatInput
                      label="Destino"
                      required
                      value={destinoOutro}
                      onChange={setDestinoOutro}
                      maxLength={255}
                      className="md:w-1/2"
                    />
                  )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start mt-2">
                    <div className="flex flex-col gap-4">
                      <CheckboxGroup
                        ...
                        required
                        actionLabel=""
                        options={toCheck(TRATAMENTOS)}
                        defaultValue={tratAfluente}
                        onChange={setTratAfluente}
                        orientation="vertical"
                      />
                      {tratAfluente.includes("Outro") && (
                        <FloatInput
                          label="Tratamento (Afluente)"
                          required
                          value={tratAfluenteOutro}
                          onChange={setTratAfluenteOutro}
                          maxLength={255}
                          className="w-full"
                        />
                      )}
                    ...
                    <div className="flex flex-col gap-4">
                      <CheckboxGroup
                        ...
                        orientation="vertical"
                      />
                      {tratEfluente.includes("Outro") && (
                        <FloatInput
                          label="Tratamento (Efluente)"
                          required
                          value={tratEfluenteOutro}
                          onChange={setTratEfluenteOutro}
                          maxLength={255}
                          className="w-full"
                        />
                      )}
          ...
          <div className="flex flex-col gap-4">
            ...
            {isSub === true && (
              <ExploracaoPecuariaInput
                ...
                required
                onChange={(entidadeSelecionada) =>
                  setExploracao(entidadeSelecionada)
                }
                onEyeClick={() => {
                  if (exploracao?.codigo)
                    alert(`Visualizar detalhes: ${exploracao.codigo}`);
                  else
                    alert(
                      "Por favor, digite ou selecione uma revendedora primeiro.",
                    );
