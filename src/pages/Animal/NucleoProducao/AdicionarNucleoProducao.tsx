...
interface ContatoFornecedorForm {
  id: string;
  tipo: string;
  valor: string;
  observacao: string;
...
interface FornecedorForm {
  id: number;
  possuiCadastro: string;
  fornecedorCod: string;
  nomeFornecedor: string;
  especie: string;
  estado: string;
  municipio: string;
  contatos: ContatoFornecedorForm[];
}
...
// ==========================================================
// PÁGINA: ADICIONAR NÚCLEO DE PRODUÇÃO (US073 - AC3)
// ==========================================================
interface PageProps {
  onLogout: () => void;
...
export function AdicionarNucleoProducaoPage({ onLogout, onNavigate }: PageProps) {
  // Informações Básicas
  const [nome, setNome] = useState("");

  // Exploração Pecuária (entidade) — define o grupo/espécie
  const [exploracao, setExploracao] = useState<ExploracaoEntidade | null>(null);
  const [modalExploracao, setModalExploracao] = useState(false);

  // Informações Complementares (Aves / Suídeos)
  const [numGalpoes, setNumGalpoes] = useState("");
  const [capacidade, setCapacidade] = useState("");
  const [possuiPiquetes, setPossuiPiquetes] = useState("");
  const [numPiquetes, setNumPiquetes] = useState("");

  // Identificação das Espécies (Abelhas)
  const [subespecies, setSubespecies] = useState<string[]>([]);
  const [origemEnxames, setOrigemEnxames] = useState<string[]>([]);

  // Material Genético
  const [materialImportado, setMaterialImportado] = useState("");
  const [paisOrigem, setPaisOrigem] = useState("");
  const [modalAberto, setModalAberto] = useState(false);
  const [idItemAtivo, setIdItemAtivo] = useState<number | null>(null);

  // Caracterização — Aves
  const [avesArea, setAvesArea] = useState("");
  const [avesClassif, setAvesClassif] = useState("");
  const [avesNomeClassif, setAvesNomeClassif] = useState("");
  const [avesCaracAdicional, setAvesCaracAdicional] = useState<string[]>([]);

  // Caracterização — Suídeos
  const [suiTipo, setSuiTipo] = useState("");
  const [suiAreaSimples, setSuiAreaSimples] = useState("");
  const [suiAreaMultipla, setSuiAreaMultipla] = useState<string[]>([]);
  const [suiClassif, setSuiClassif] = useState("");

  // Caracterização — Abelhas
  const [abFinalidade, setAbFinalidade] = useState<string[]>([]);
  const [abArea, setAbArea] = useState<string[]>([]);
  const [abDistancia, setAbDistancia] = useState("");
  const [abFluxo, setAbFluxo] = useState("");
  const [abTipoProducao, setAbTipoProducao] = useState("");
  const [abPossuiCasa, setAbPossuiCasa] = useState("");
  const [produtos, setProdutos] = useState<any[]>([]);
  const [realizaAlimentacao, setRealizaAlimentacao] = useState("");
  const [alimentacoes, setAlimentacoes] = useState<any[]>([]);
  const [tipoDestinoProd, setTipoDestinoProd] = useState<string[]>([]);
  const [destinoOutro, setDestinoOutro] = useState("");
  const [escalaComercio, setEscalaComercio] = useState<string[]>([]);
  const [comercioOutro, setComercioOutro] = useState("");
  const [tipoComercioRainhas, setTipoComercioRainhas] = useState<string[]>([]);
  const [comercioRainhasOutro, setComercioRainhasOutro] = useState("");
  const [modalProduto, setModalProduto] = useState(false);
  const [abSubespecie, setAbSubespecie] = useState("");
  ...
  const [possuiMaterialImportado, setPossuiMaterialImportado] = useState<string>("");

  // Sucesso
  const [isSucesso, setIsSucesso] = useState(false);

  // ---- Flags derivadas do grupo/espécie ----
  ...
  const origemTemCompra = origemEnxames.includes("Compra");
  ...
  const handleAdicionar = () => {
    const fornecedorComContatoInvalido = origemTemCompra && fornecedores.some((fornecedor) =>
      fornecedor.possuiCadastro === "Não" && (
        fornecedor.contatos.length === 0 ||
        fornecedor.contatos.some((contato) => !contato.tipo.trim() || !contato.valor.trim())
      )
    );
    ...
    <div className="min-h-screen bg-[#f2f3f5]">
      ...
      <main className="max-w-5xl mx-auto px-4 md:px-6 py-6 flex flex-col gap-4">
        ...
        <div>
          ...
          >
            <ArrowLeft size={15} />
            Todos os Núcleos de Produção
          </button>
          <div className="flex justify-between items-center w-full">
            <h1 className="text-2xl font-semibold text-gray-900">Adicionar Núcleo de Produção</h1>
            <button
              type="button"
        ...
        <Section title="Informações da Exploração Pecuária">
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              {/* Se tiver exploração, muda dinamicamente a quantidade de colunas do grid */}
             {/* Se tiver exploração, vira um grid de colunas. Caso contrário, vira apenas um bloco flex de linha cheia */}
              <div className={exploracao ? "grid grid-cols-1 md:grid-cols-2 gap-4 items-center" : "w-full"}>
                ...
                {/* O campo de Espécie só renderiza se 'exploracao' existir */}
                {exploracao && (
                  <div className="flex items-center gap-2 animate-fadeIn w-full">
                    {/* Se for Abelhas, divide o espaço interno para mostrar a Espécie e a Subespécie (ambas desabilitadas) antes do olhinho */}
                    {(exploracao.especie === "Abelha com Ferrão" || exploracao.especie === "Abelha sem Ferrão" || exploracao.especie === "Abelhas") ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 flex-1">
                        ...
                        <FloatInput
                          label="Raça"
                          required
                          value={abSubespecie || "Africanizada"}
                          onChange={() => { }}
                          disabled
                        />
                      </div>
                    ) : (
                      /* Caso contrário, renderiza apenas o campo de espécie padrão ocupando o espaço total antes do olhinho */
                      <div className="flex-1">
                        <FloatInput
                          label="Espécie Explorada"
                          required
                    ...
                    {/* O botão do olhinho fica sempre no final da linha, após os inputs */}
                              ...
                              {f.possuiCadastro === "Não" && (
                                <div className="flex flex-col gap-4 w-full animate-fadeIn">
                                  ...
                                  <div className="flex flex-col gap-3 w-full">
                                    ...
                                    {(f.contatos || []).map((contato, contatoIndex) => (
                                      <div key={contato.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-start rounded-xl relative animate-fadeIn">
                                        <FloatSelect
                                          ...
                                          onChange={(v) => setFornecedores((prev) => prev.map((x) => x.id === f.id
                                            ? { ...x, contatos: (x.contatos || []).map((c, i) => i === contatoIndex ? { ...c, tipo: v } : c) }
                                            : x))}
                                        ...
                                        <FloatInput
                                          ...
                                          value={contato.valor}
                                          onChange={(v) => setFornecedores((prev) => prev.map((x) => x.id === f.id
                                            ? { ...x, contatos: (x.contatos || []).map((c, i) => i === contatoIndex ? { ...c, valor: v } : c) }
                                            : x))}
                                        ...
                                        <div className={`${contatoIndex === 0 ? "md:col-span-5" : "md:col-span-4"} relative border border-gray-300 rounded-md h-24 flex flex-col justify-between p-2.5 bg-white`}>
                                          ...
                                          <textarea
                                            ...
                                            onChange={(e) => setFornecedores((prev) => prev.map((x) => x.id === f.id
                                              ? { ...x, contatos: (x.contatos || []).map((c, i) => i === contatoIndex ? { ...c, observacao: e.target.value } : c) }
                                              : x))}
                                          ...
                                          <div className="md:col-span-1 flex justify-center pt-1.5">
                                            <button
                                              ...
                                              onClick={() => setFornecedores((prev) => prev.map((x) => x.id === f.id
                                                ? { ...x, contatos: (x.contatos || []).filter((c) => c.id !== contato.id) }
                                                : x))}
              ...
              <Section title="Caracterização do Núcleo">
                <div className="flex flex-col gap-5">
                  ...
                  <div className="flex gap-3">
                    ...
                    <FloatSelect label="Apiário/Meliponário fixo ou migratório?" required value={abFluxo} onChange={setAbFluxo} options={toOptions(AB_FLUXO)} className="w-[320px] flex-shrink-0" />
                  </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 text-center">
            ...
            <p className="text-sm text-gray-500 mt-1">
              {nome ? `${nome}` : "O núcleo"} foi adicionado como um novo Núcleo de Produção.
            </p>
              ...
              <button
                onClick={() => {
                  ...
                  onNavigate("visualizar-nucleo-producao", {
                    ...
                    // Dados vindos do objeto exploracao
                    grupo: exploracao?.grupo,
                    especie: exploracao?.especie,
                    exploracaoCodigo: exploracao?.codigo,
                    estabNome: exploracao?.estabNome,
                    estabCodigo: exploracao?.estabCodigo,
                    produtores: exploracao?.produtores ?? [],
                    ...
                    subespecies,
                    origemEnxames,
                    possuiMaterialImportadoAbelha: possuiMaterialImportado, // Mapeado da sua state possuiMaterialImportado
                    fornecedores,

                    // Caracterizações (Aves)
                    avesArea,
                    avesClassif,
                    avesNomeClassif,
                    avesCaracAdicional,

                    // Caracterizações (Suídeos)
                    suiTipo,
                    suiAreaSimples,
                    suiAreaMultipla,
                    suiClassif,

                    // Caracterizações (Abelhas)
                    abFinalidade,
                    abArea,
                    abDistancia,
                    abFluxo,
