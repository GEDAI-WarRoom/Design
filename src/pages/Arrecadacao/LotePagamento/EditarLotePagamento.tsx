...
export function EditarLotePagamentoPage({ dados, onLogout, onNavigate }: { dados?: any; onLogout: () => void; onNavigate: (s: string, d?: any) => void; }) {
  const [isSucesso, setIsSucesso] = useState(false);
  const [situacao, setSituacao] = useState(dados?.situacao || "Aberto");
      ...
      <main className="max-w-[1088px] mx-auto px-4 md:px-6 py-6 flex flex-col gap-4">
        <div>
          ...
          <div className="flex justify-between items-center w-full">
            <h1 className="text-2xl font-semibold text-gray-900">Editar Lote de Pagamento</h1>
        ...
        <Section title="Informações do Lote">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <FloatInput label="Número do Lote" value={dados?.numeroLote || "-"} disabled onChange={()=>{}} />
            <FloatInput label="Documento" value={dados?.documento || "-"} disabled onChange={()=>{}} />
            <FloatSelect label="Situação" required value={situacao} onChange={setSituacao} options={[{ value: "Aberto", label: "Aberto" }, { value: "Processado", label: "Processado" }, { value: "Cancelado", label: "Cancelado" }]} />
          </div>
        </Section>
      ...
      {isSucesso && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 text-center">
            ...
            <p className="text-sm text-gray-500 mt-1">O Lote foi atualizado com sucesso.</p>
            <div className="flex gap-3 justify-center mt-6">
              <button onClick={() => { setIsSucesso(false); onNavigate("lote-pagamento"); }} className="px-5 h-11 rounded-md border border-[#1A7A3C] text-[#1A7A3C] text-sm font-semibold hover:bg-green-50 transition">Voltar</button>
              <button onClick={() => { setIsSucesso(false); onNavigate("visualizar-lote-pagamento", dados); }} className="px-5 h-11 rounded-md bg-[#1A7A3C] hover:bg-[#15612F] text-white text-sm font-semibold transition">Visualizar</button>
            </div>
          </div>
        </div>
      )}
