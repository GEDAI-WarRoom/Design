...
interface DocumentoData {
  ...
  lote: LotePagamento;
  origem?: "adicionar-lote-pagamento" | "editar-lote-pagamento";
}
...
export function VisualizarDocumentoLotePagamentoPage({ onLogout, onNavigate, dados }: { onLogout: () => void; onNavigate: (screen: any, data?: any) => void; dados?: DocumentoData }) {
  ...
  const { documento, lote, origem } = dados;
  return (
    <div className="min-h-screen bg-[#f2f3f5]">
      ...
      <main className="mx-auto flex max-w-4xl flex-col gap-5 px-4 py-6 md:px-6">
        <button type="button" onClick={() => onNavigate(origem ?? "visualizar-lote-pagamento", lote)} className="flex items-center gap-1 text-sm font-semibold text-[#1A7A3C] hover:opacity-70"><ArrowLeft size={15} />Voltar ao Lote de Pagamento</button>
        ...
        <section className="rounded-xl bg-white p-6 shadow-sm">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <FloatInput label="Finalidade" value={documento.finalidade} disabled />
            <FloatInput label="Espécie" value={documento.especie} disabled />
            <FloatInput label="Total de animais" value={String(documento.totalAnimais)} disabled />
            <FloatInput label="Valor" value={formatarMoedaLote(documento.valor)} disabled />
            <FloatInput label="Data da emissão" value={formatarDataLote(documento.dataEmissao)} disabled />
            <FloatInput label="Situação" value={documento.status} disabled />
          </div>
        </section>
      </main>
    </div>
