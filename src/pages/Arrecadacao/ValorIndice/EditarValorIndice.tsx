...
export function EditarValorIndicePage({ dados, onLogout, onNavigate }: { dados?: any; onLogout: () => void; onNavigate: (s: string, d?: any) => void; }) {
  ...
  return (
    <div className="min-h-screen bg-[#f2f3f5]">
      ...
      <main className="max-w-[1088px] mx-auto px-4 md:px-6 py-6 flex flex-col gap-4">
        <div>
          <button type="button" onClick={() => onNavigate("valor-indice")} className="flex items-center gap-1 text-sm mb-3 transition hover:opacity-70 font-semibold" style={{ color: GREEN }}>
            <ArrowLeft size={15} /> Todos os Valores de Índice
          </button>
          <div className="flex justify-between items-center w-full">
            <h1 className="text-2xl font-semibold text-gray-900">Editar Valor de Índice</h1>
        ...
        <Section title="Informações do Valor">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <FloatInput label="Índice Relacionado" value={dados?.indice || "UFEMG"} disabled onChange={()=>{}} />
            <FloatInput label="Valor" required placeholder="0,00" value={valor} onChange={(v) => setValor(v.replace(/[^0-9,]/g, ""))} />
            <FloatInput label="Ano" required placeholder="0000" value={ano} onChange={(v) => setAno(v.replace(/\D/g, "").slice(0, 4))} maxLength={4} />
            <FloatSelect label="Mês" required value={mes} onChange={setMes} options={MESES.map(m => ({ value: m, label: m }))} />
      ...
      {isSucesso && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 text-center">
            ...
            <p className="text-sm text-gray-500 mt-1">O Valor de Índice foi atualizado.</p>
            <div className="flex gap-3 justify-center mt-6">
              <button onClick={() => { setIsSucesso(false); onNavigate("valor-indice"); }} className="px-5 h-11 rounded-md border border-[#1A7A3C] text-[#1A7A3C] text-sm font-semibold hover:bg-green-50 transition">Voltar</button>
              <button onClick={() => { setIsSucesso(false); onNavigate("visualizar-valor-indice", dados); }} className="px-5 h-11 rounded-md bg-[#1A7A3C] hover:bg-[#15612F] text-white text-sm font-semibold transition">Visualizar</button>
            </div>
          </div>
        </div>
      )}
