        ...
        <div className="bg-white rounded-xl shadow-sm p-6 flex flex-col gap-5">
          <div className="flex gap-3 items-stretch w-full">
            <div className="flex-1 bg-white border border-gray-300 rounded-md px-3 h-12 transition-all relative flex items-end pb-1.5 focus-within:border-[#1A7A3C] focus-within:ring-1 focus-within:ring-[#1A7A3C]">
              <label className={`absolute left-3 transition-all duration-200 pointer-events-none ${focusNome || nomeIndice ? "top-1 text-[10px] text-gray-400 font-medium" : "top-1/2 -translate-y-1/2 text-sm text-gray-400"}`}>
                Nome do Índice
              </label>
            ...
            <button
              ...
              style={{ backgroundColor: showFilters ? "transparent" : GREEN, borderColor: GREEN, color: showFilters ? GREEN : "#ffffff" }}
            >
          ...
          {showFilters && (
            <div className="animate-fadeIn flex flex-col lg:flex-row items-end gap-3 w-full">
              <div className="w-full lg:flex-1">
                <FloatSelect label="Situação" value={situacaoFiltro} onChange={setSituacaoFiltro} options={SITUACOES_OPCOES} />
              </div>
              <button
          ...
          {temFiltroAtivo && (
            <div className="flex flex-wrap gap-2 animate-fadeIn">
              {nomeIndice && <Chip label={`Índice: ${nomeIndice}`} onRemove={() => setNomeIndice("")} />}
              {situacaoFiltro && <Chip label={`Situação: ${situacaoFiltro}`} onRemove={() => setSituacaoFiltro("")} />}
            </div>
          )}
          ...
          {!hasSearched ? (
            <div className="py-5 text-center"><p className="text-sm text-gray-500">Busque por um índice utilizando o campo de busca e os filtros acima.</p></div>
          ) : total === 0 ? (
            <div className="py-5 text-center"><p className="text-sm text-gray-500">Nenhum resultado foi encontrado.</p></div>
          ) : (
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-gray-300">
                      ...
                      <th className="px-4 py-3 w-[100px] text-right font-semibold text-gray-600 uppercase">Ações</th>
                    </tr>
                  </thead>
                  <tbody>
                    {pagina.map((item) => (
                      <tr key={item.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition">
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex items-center justify-end gap-1">
                            {/* O Olhinho agora vai para visualizar-indice */}
                            <button onClick={() => onNavigate("visualizar-indice", item)} className="p-2 rounded-md hover:bg-green-50 transition" style={{ color: GREEN }} title="Visualizar">
                              <ViewIcon size={18} />
                            </button>
                            {/* O Lápis vai para adicionar-indice (que trata como edição) */}
                            <button onClick={() => onNavigate("adicionar-indice", item)} className="p-2 rounded-md hover:bg-green-50 transition" style={{ color: GREEN }} title="Editar">
                              <Pencil size={17} />
                            </button>
                          </div>
                        </td>
