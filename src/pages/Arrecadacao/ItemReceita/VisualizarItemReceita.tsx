...
export function VisualizarItemReceitaPage({ dados, onLogout, onNavigate }: { dados?: any; onLogout: () => void; onNavigate: (s: string, d?: any) => void; }) {
  const item = dados || {};
      ...
      <main className="max-w-[1088px] mx-auto px-4 md:px-6 py-6 flex flex-col gap-4">
        <div>
          ...
          <div className="flex justify-between items-center w-full">
            ...
            <button type="button" onClick={() => onNavigate("editar-item-receita", item)} className="px-5 h-10 bg-[#1A7A3C] hover:bg-[#15612F] text-white text-xs font-bold rounded-md transition shadow-sm flex items-center gap-2">
        ...
        <Section title="Informações Básicas">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <FloatInput label="Código do Item" value={item.codigo || "-"} disabled onChange={() => {}} />
            <FloatInput label="Nome do Item" value={item.nome || "-"} disabled onChange={() => {}} />
            <FloatInput label="Valor Padrão (R$)" value={item.valor || "-"} disabled onChange={() => {}} />
            <FloatInput label="Situação" value={item.situacao || "Ativo"} disabled onChange={() => {}} />
          </div>
        </Section>

        <Section title="Observações">
          <LargeTextArea label="Observações" value={item.observacao || "Nenhuma observação."} disabled onChange={() => {}} />
        </Section>
      </main>
    </div>
  );
