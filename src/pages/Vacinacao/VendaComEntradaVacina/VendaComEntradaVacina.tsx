...
export function VendaComEntradaVacinaPage({ onLogout, onNavigate }: PageProps) {
  ...
  const filtrados = VENDAS_MOCK.filter((v) => {
    ...
    return (
      matchRevendedora && matchFornecedor && matchNF &&
      matchPartida && matchDoenca && matchTipoVacina && matchSituacao
    );
  });
      ...
      <main className="max-w-[1088px] mx-auto px-4 md:px-6 py-6">
        ...
        <div className="mb-5">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-semibold text-gray-900">Venda com Entrada de Vacina</h1>
            <button
              onClick={() => onNavigate("adicionar-venda-entrada-vacina")}
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-6 border-b border-gray-100 bg-white">
            ...
            {(revendedora || fornecedor || numeroNotaFiscal || numeroPartida || doenca || tipoVacina || situacao) && (
              <div className="flex flex-wrap gap-2 mt-4 animate-fadeIn">
                ...
                {fornecedor && <Chip label={`Fornecedor: ${fornecedor.nome}`} onRemove={() => setFornecedor(null)} />}
            ...
            {!hasSearched ? (
              <div className="p-12 text-center bg-white">
                <p className="text-sm text-gray-500">
                  Busque por venda com entrada de vacina utilizando os filtros acima
                </p>
              </div>
            ) : total === 0 ? (
              <div className="p-12 text-center bg-white border-t border-gray-100">
                        ...
                        <tr key={v.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition">
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-1 justify-end">
                              <button onClick={() => onNavigate("visualizar-venda-entrada-vacina", v)} className="p-2 rounded-md hover:bg-green-50 transition" style={{ color: GREEN }} title="Visualizar">
                                <ViewIcon size={18} />
                              </button>
                              <button onClick={() => onNavigate("editar-venda-entrada-vacina", v)} className="p-2 rounded-md hover:bg-green-50 transition" style={{ color: GREEN }} title="Editar">
                                <Pencil size={17} />
                              </button>
                            </div>
                          </td>
                ...
                <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 text-sm text-gray-500 bg-white">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={pageAtual === 1} className="p-1.5 rounded-md hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed transition">
                        <ChevronLeft size={18} />
                      </button>
                      <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={pageAtual === totalPages} className="p-1.5 rounded-md hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed transition">
