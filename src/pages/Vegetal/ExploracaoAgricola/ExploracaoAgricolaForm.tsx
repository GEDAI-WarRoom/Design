...
import { ArrowLeft, CalendarDays, Check, ChevronDown, ChevronUp, Info, MapPin } from "lucide-react";
import { Navbar } from "../../../components/Navbar";
import { CustomRadio, FloatInput, FloatSelect, LargeTextArea, UploadField } from "../../../components/ui/FormKit";
...
import {
  ...
  type AnexoExploracao, type ExploracaoAgricola, type ProdutorAgricola,
...
export function ExploracaoAgricolaForm({ mode, dados, onLogout, onNavigate, acaoHistorico, avisoHistorico, esconderNavbar = false, podeEditar = true }: { mode: Mode; dados?: ExploracaoAgricola; onLogout: () => void; onNavigate: (screen: any, data?: any) => void; acaoHistorico?: ReactNode; avisoHistorico?: ReactNode; esconderNavbar?: boolean; podeEditar?: boolean }) {
  ...
  const [produtores, setProdutores] = useState<Array<ProdutorAgricola | null>>(inicial.produtores?.length ? inicial.produtores : [null]);
  ...
  const registroAtual = useMemo<ExploracaoAgricola | null>(() => {
    ...
    return {
      ...
      unidadeArea, areaProdutiva, areaUtil, produtores: produtores.filter((item): item is ProdutorAgricola => Boolean(item)), variedade, unidadeMedida, dataPlantio, localizacaoLivro,
      ...
      latitude, longitude, anexos, observacao, situacao: situacaoEfetiva,
    };
  }, [dados, estabelecimento, unidadeArea, areaProdutiva, areaUtil, produtores, variedade, unidadeMedida, dataPlantio, localizacaoLivro, unidadeProducao, dataVencimento, necessitaRT, responsavelTecnico, latitude, longitude, anexos, observacao, situacaoEfetiva]);
  ...
  const validarESalvar = () => {
    ...
    if (!produtores.some(Boolean)) novos.push("Adicione pelo menos um produtor.");
  ...
  return <div className="min-h-screen bg-[#f2f3f5]">
    ...
    <main data-situacao-container data-situacao={situacaoEfetiva} className="mx-auto flex max-w-[1088px] flex-col gap-4 px-4 py-6 md:px-6">
      <div><button type="button" onClick={() => onNavigate("exploracao-agricola")} className="mb-3 flex items-center gap-1 text-sm text-[#1A7A3C]"><ArrowLeft size={15} />Todas as Explorações Agrícolas</button><div className="flex items-center justify-between gap-4"><h1 className="text-2xl font-semibold text-gray-900">{titulo}</h1><div className="flex gap-2">{mode === "view" ? <>{dados?.unidadeProducao === "Sim" && <button type="button" onClick={() => onNavigate("editar-exploracao-agricola", dados)} className="h-10 rounded-md border border-[#1A7A3C] px-4 text-xs font-bold text-[#1A7A3C]">Manutenção</button>}{podeEditar && <button type="button" onClick={() => onNavigate("editar-exploracao-agricola", dados)} className="h-10 rounded-md bg-[#1A7A3C] px-5 text-xs font-bold text-white">Editar</button>}{acaoHistorico}</> : <><button type="button" onClick={validarESalvar} className="h-10 rounded-md bg-[#1A7A3C] px-5 text-xs font-bold text-white hover:bg-[#15612F]">{mode === "edit" ? "Salvar" : "Adicionar"}</button>{acaoHistorico}</>}</div></div></div>
      {avisoHistorico}
      ...
      <Section title="Produtores"><DynamicListWrapper items={produtores} behavior="at-least-one" variant="plain" itemLabel="Produtor" addButtonLabel="Adicionar Produtor" disabled={somenteLeitura} onAddItem={() => setProdutores((items) => [...items, null])} onRemoveItem={(index) => setProdutores((items) => items.filter((_, i) => i !== index))}>{(item, index) => <EntitySearchInput label="Produtor" required disabled={somenteLeitura} value={item?.nome ?? ""} data={PRODUTORES_AGRICOLAS_MOCK.filter((p) => !produtores.some((selecionado, i) => i !== index && selecionado?.id === p.id))} searchKeys={["nome", "documento"]} columns={[{ label: "CPF/CNPJ", key: "documento" }, { label: "Nome/Razão Social", key: "nome" }]} icon={<img src={Icons.iconeProdutorUrl} className="h-5 w-5 object-contain" alt="" />} title="Buscar Produtor" subtitle="Busque por um produtor cadastrado:" onChange={(valor) => setProdutores((items) => items.map((atual, i) => i === index ? valor : atual))} />}</DynamicListWrapper></Section>
