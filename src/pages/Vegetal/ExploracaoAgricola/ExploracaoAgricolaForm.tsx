...
import { ArrowLeft, CalendarDays, Check, ChevronDown, ChevronUp, FileText, Info, Link2, Map, MoreVertical, ShieldCheck, UserRound } from "lucide-react";
import { Navbar } from "../../../components/Navbar";
import { AccordionCardGroup, CustomRadio, FieldTooltip, FloatInput, FloatSelect, LargeTextArea, Tabs, UploadField } from "../../../components/ui/FormKit";
import { DynamicListWrapper, EntitySearchInput, MapModal } from "../../../components/ui/EntitySearch";
...
import {
  ...
  TIPOS_PRODUTOR, type AnexoExploracao, type ExploracaoAgricola, type ManutencaoExploracaoAgricola, type ProdutorAgricola, type ResponsavelTecnico, type TipoProdutor,
} from "./exploracaoAgricolaData";
import { listarUnidadesMedida as listarUnidadesCadastradas } from "../../Geral/UnidadeMedida/unidadeMedidaData";

const GREEN = "#1A7A3C";
const HOJE = new Date().toISOString().slice(0, 10);
type Mode = "create" | "view" | "edit";
type ProdutorItem = { uid: string; produtor: ProdutorAgricola | null; tipo: TipoProdutor };
...
export function ExploracaoAgricolaForm({ mode, dados, onLogout, onNavigate, acaoHistorico, avisoHistorico, esconderNavbar = false, podeEditar = true, abaAtiva = "cadastro", onMudarAba }: { mode: Mode; dados?: ExploracaoAgricola; onLogout: () => void; onNavigate: (screen: any, data?: any) => void; acaoHistorico?: ReactNode; avisoHistorico?: ReactNode; esconderNavbar?: boolean; podeEditar?: boolean; abaAtiva?: string; onMudarAba?: (aba: string) => void }) {
  ...
  const [produtores, setProdutores] = useState<ProdutorItem[]>(
    inicial.produtores?.length
      ? inicial.produtores.map((produtor) => ({ uid: `produtor-${produtor.id}`, produtor, tipo: produtor.tipoProdutor ?? "Proprietário" }))
      : [{ uid: `produtor-${Date.now()}`, produtor: null, tipo: "Proprietário" }],
  );
  const [variedade, setVariedade] = useState<any>(inicial.variedade ?? null);
  const unidadesMedida = listarUnidadesCadastradas().filter((item) => item.situacao === "Ativo" && item.tipos.includes("Vegetal"));
  const unidadeInicial = unidadesMedida.find((item) => item.sigla === inicial.unidadeMedida || item.nome === inicial.unidadeMedida) ?? null;
  const [unidadeMedida, setUnidadeMedida] = useState<any>(unidadeInicial);
  ...
  const registroAtual = useMemo<ExploracaoAgricola | null>(() => {
    ...
    return {
      ...
      unidadeArea, areaProdutiva, areaUtil, produtores: produtores.map((item) => item.produtor ? { ...item.produtor, tipoProdutor: item.tipo } : null).filter((item): item is ProdutorAgricola => Boolean(item)), variedade, unidadeMedida: unidadeMedida?.sigla ?? "", dataPlantio, localizacaoLivro,
  ...
  const adicionarCoordenadas = () => setMapaAberto(true);
  ...
  return <div className="min-h-screen bg-[#f2f3f5]">
    ...
    <main data-situacao-container data-situacao={situacaoEfetiva} data-situacao-tab={abaAtiva} className="mx-auto flex max-w-[1088px] flex-col gap-4 px-4 py-6 md:px-6">
      ...
      <Section title="Produtores"><div className="flex flex-col gap-6"><div><div className="mb-3 flex items-center gap-2 text-sm font-semibold text-gray-800">Produtor Titular <FieldTooltip text="O produtor titular é responsável pela renovação do cadastro da unidade de produção." /></div><div className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_2fr]"><FloatSelect label="Tipo de Produtor" required disabled={somenteLeitura} value={produtores[0]?.tipo ?? "Proprietário"} onChange={(tipo) => setProdutores((items) => items.map((item, index) => index === 0 ? { ...item, tipo: tipo as TipoProdutor } : item))} options={TIPOS_PRODUTOR.map((value) => ({ value, label: value }))} /><EntitySearchInput label="Produtor" required disabled={somenteLeitura} value={produtores[0]?.produtor?.nome ?? ""} data={PRODUTORES_AGRICOLAS_MOCK} searchKeys={["nome", "documento"]} columns={[{ label: "CPF/CNPJ", key: "documento" }, { label: "Nome/Razão Social", key: "nome" }]} icon={<img src={Icons.iconeProdutorUrl} className="h-5 w-5 object-contain" alt="" />} title="Buscar Produtor" subtitle="Busque por um produtor cadastrado:" onChange={(valor) => setProdutores((items) => items.map((atual, index) => index === 0 ? { ...atual, produtor: valor } : atual))} /></div></div><div className="border-t border-gray-100 pt-6"><p className="mb-4 text-sm font-semibold text-gray-800">Outros Produtores</p><DynamicListWrapper items={produtores.slice(1)} behavior="zero-or-more" variant="plain" itemLabel="Produtor" addButtonLabel="Adicionar Produtor" disabled={somenteLeitura} onAddItem={() => setProdutores((items) => [...items, { uid: `produtor-${Date.now()}`, produtor: null, tipo: "Proprietário" }])} onRemoveItem={(index) => setProdutores((items) => items.filter((_, i) => i !== index + 1))}>{(item, index) => <div className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_2fr]"><FloatSelect label="Tipo de Produtor" required disabled={somenteLeitura} value={item.tipo} onChange={(tipo) => setProdutores((items) => items.map((atual, i) => i === index + 1 ? { ...atual, tipo: tipo as TipoProdutor } : atual))} options={TIPOS_PRODUTOR.map((value) => ({ value, label: value }))} /><EntitySearchInput label="Produtor" required disabled={somenteLeitura} value={item.produtor?.nome ?? ""} data={PRODUTORES_AGRICOLAS_MOCK.filter((p) => !produtores.some((selecionado, i) => i !== index + 1 && selecionado.produtor?.id === p.id))} searchKeys={["nome", "documento"]} columns={[{ label: "CPF/CNPJ", key: "documento" }, { label: "Nome/Razão Social", key: "nome" }]} icon={<img src={Icons.iconeProdutorUrl} className="h-5 w-5 object-contain" alt="" />} title="Buscar Produtor" subtitle="Busque por um produtor cadastrado:" onChange={(valor) => setProdutores((items) => items.map((atual, i) => i === index + 1 ? { ...atual, produtor: valor } : atual))} /></div>}</DynamicListWrapper></div></div></Section>

      <Section title="Informações da Cultura Explorada"><div className="grid grid-cols-1 gap-4 md:grid-cols-2"><EntitySearchInput label="Variedade de Cultura" required disabled={somenteLeitura} value={variedade ? `${variedade.cultura} - ${variedade.nome}` : ""} data={VARIEDADES_CULTURA_MOCK} searchKeys={["cultura", "nome"]} columns={[{ label: "Cultura", key: "cultura" }, { label: "Variedade", key: "nome" }]} icon={<img src={Icons.iconeCulturaUrl} className="h-5 w-5 object-contain" alt="" />} title="Buscar Variedade de Cultura" subtitle="Busque por uma variedade cadastrada:" onChange={(item) => { setVariedade(item); setUnidadeMedida(unidadesMedida.find((unidade) => unidade.sigla === item?.unidadePadrao) ?? null); setResponsavelTecnico(null); }} /><EntitySearchInput label="Unidade de Medida Padrão" required disabled={somenteLeitura} value={unidadeMedida ? `${unidadeMedida.nome} (${unidadeMedida.sigla})` : ""} data={unidadesMedida} searchKeys={["nome", "sigla"]} columns={[{ label: "Nome", key: "nome" }, { label: "Sigla", key: "sigla" }]} title="Buscar Unidade de Medida" subtitle="Busque por uma unidade de medida cadastrada:" onChange={setUnidadeMedida} /></div></Section>

      <Section title="Informações Complementares"><div className="flex flex-col gap-6"><div className="grid grid-cols-1 gap-4 md:grid-cols-2"><FloatInput label="Data de Plantio" required disabled={somenteLeitura} type="date" max={HOJE} value={dataPlantio} onChange={setDataPlantio} /><FloatInput label="Localização do Livro" required disabled={somenteLeitura} value={localizacaoLivro} onChange={setLocalizacaoLivro} maxLength={255} /></div><div className="flex items-start gap-2"><SimNao label="É uma Unidade de Produção? (Necessita de CFO)" name="unidade-producao" disabled={somenteLeitura} value={unidadeProducao} onChange={setUnidadeProducao} /><FieldTooltip text="Nos casos em que houver Necessidade de CFO (Certificado Fitossanitário de Origem), o cadastro será caracterizado como uma Unidade de Produção, necessitando de atualizações de manutenção." /></div>{unidadeProducao === "Sim" && <FloatInput label="Data de Vencimento" value={formatarData(dataVencimento)} disabled icon={<CalendarDays size={18} />} onChange={() => {}} />}<SimNao label="Necessita de Responsável Técnico?" name="necessita-rt" disabled={somenteLeitura} value={necessitaRT} onChange={(v) => { setNecessitaRT(v); if (v === "Não") setResponsavelTecnico(null); }} />{necessitaRT === "Sim" && <EntitySearchInput label="Responsável Técnico" required disabled={somenteLeitura} value={responsavelTecnico?.nome ?? ""} data={RESPONSAVEIS_TECNICOS_MOCK} searchKeys={["nome", "documento", "codigo"]} columns={[{ label: "Nome", key: "nome" }, { label: "CPF", key: "documento" }, { label: "Registro", key: "codigo" }]} icon={<img src={Icons.iconeProfissionalVegetalUrl} className="h-5 w-5 object-contain" alt="" />} title="Buscar Responsável Técnico" subtitle="Busque por um profissional habilitado:" onChange={setResponsavelTecnico} />}{rtIncompativel && <p className="text-xs text-amber-700" role="alert">O responsável técnico não possui habilitação para todas as pragas vinculadas à cultura. O cadastro ficará suspenso.</p>}</div></Section>

      <Section title="Informações de Localização"><p className="mb-4 text-sm font-semibold text-gray-800">Geolocalização</p>{!latitude || !longitude ? <button type="button" disabled={somenteLeitura} onClick={adicionarCoordenadas} className="flex h-11 w-full items-center justify-center gap-2 rounded-md border border-[#1A7A3C] text-sm font-semibold text-[#1A7A3C] disabled:cursor-not-allowed disabled:opacity-50"><Map size={17} />Adicionar Coordenadas</button> : <div className="grid grid-cols-1 gap-4 md:grid-cols-2"><button type="button" disabled={somenteLeitura} onClick={adicionarCoordenadas} className="flex h-12 items-center justify-center gap-2 rounded-md border border-[#1A7A3C] text-sm font-semibold text-[#1A7A3C] disabled:cursor-not-allowed disabled:opacity-50"><Map size={17} /></button><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><FloatInput label="Latitude" required disabled value={latitude} onChange={setLatitude} /><FloatInput label="Longitude" required disabled value={longitude} onChange={setLongitude} /></div></div>}</Section>

      <Section title="Anexos"><DynamicListWrapper items={anexos} behavior="zero-or-more" variant="plain" itemLabel="Anexo" addButtonLabel="Adicionar Anexo" disabled={somenteLeitura} onAddItem={() => setAnexos((items) => [...items, { id: String(Date.now()), nome: "", descricao: "" }])} onRemoveItem={(index) => setAnexos((items) => items.filter((_, i) => i !== index))}>{(anexo, index) => <div className="flex flex-col gap-3 md:flex-row"><UploadField label="Documento" fileName={anexo.nome} disabled={somenteLeitura} onSelectFile={() => setAnexos((items) => items.map((item, i) => i === index ? { ...item, nome: `documento_exploracao_${index + 1}.pdf` } : item))} /><div className="flex-1"><FloatInput label="Descrição" disabled={somenteLeitura} value={anexo.descricao} onChange={(value) => setAnexos((items) => items.map((item, i) => i === index ? { ...item, descricao: value } : item))} maxLength={255} /></div></div>}</DynamicListWrapper></Section>
      <Section title="Observações"><LargeTextArea label="Observações" disabled={somenteLeitura} value={observacao} onChange={setObservacao} maxLength={1500} /></Section>
      </>}
    ...
    {mapaAberto && !somenteLeitura && <MapModal initialLat={latitude} initialLng={longitude} onClose={() => setMapaAberto(false)} onConfirm={(lat, lng) => { setLatitude(lat); setLongitude(lng); setMapaAberto(false); }} />}
