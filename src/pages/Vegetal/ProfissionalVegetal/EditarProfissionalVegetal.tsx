...
export function EditarProfissionalVegetalPage({ dados, onLogout, onNavigate }: { dados?: any; onLogout: () => void; onNavigate: (screen: string, data?: any) => void; }) {
  ...
  const [registro, setRegistro] = useState(dados?.registro || "");
      ...
      <main className="max-w-[1088px] mx-auto px-4 md:px-6 py-6 flex flex-col gap-4">
        ...
        <Section title="Informações Básicas">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <FloatInput label="Registro Profissional (CREA)" value={registro} onChange={setRegistro} />
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 text-center">
            ...
            <div className="flex gap-3 justify-center mt-6">
              ...
              <button onClick={() => { setIsSucesso(false); onNavigate("visualizar-profissional-vegetal", dados); }} className="px-5 h-11 rounded-md bg-[#1A7A3C] hover:bg-[#15612F] text-white text-sm font-semibold transition">Visualizar</button>
            </div>
          </div>
        </div>
      )}
