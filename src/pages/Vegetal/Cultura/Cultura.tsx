...
function SituacaoBadge({ situacao }: { situacao: Cultura["situacao"] }) {
  const map = {
...
export function CulturaPage({ onLogout, onNavigate }: PageProps) {
  // ---- Busca principal ----
  const [busca, setBusca] = useState(""); // Nome da Cultura
  ...
  const filtrados = CULTURAS_MOCK.filter((c) => {
      ...
      <main className="max-w-[1300px] mx-auto px-4 md:px-6 py-6">
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-semibold text-gray-900">Cultura</h1>
            <button onClick={() => onNavigate("adicionar-cultura")} className="px-5 py-3 rounded-md text-white text-sm font-semibold transition hover:opacity-90 active:scale-[0.98]" style={{ backgroundColor: GREEN }}>
              Adicionar Novo
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 flex flex-col gap-5">
            ...
            <div className="animate-fadeIn flex flex-col gap-3 w-full">
              <div className="flex flex-col lg:flex-row items-end gap-3 w-full">
                <div className="w-full lg:flex-1">
                  <FloatSelect label="Tipo de Cultura" value={tipo} onChange={(v) => { setTipo(v); setErroVazio(false); }} options={TIPOS_CULTURA} />
                </div>
                <div className="w-full lg:flex-1">
          ...
          {!hasSearched ? (
            <div className="py-12 text-center">
              <p className="text-sm text-gray-500">Busque por cultura utilizando o campo de busca e os filtros acima.</p>
            </div>
          ) : total === 0 ? (
            <div className="py-12 text-center">
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className=" border-b border-gray-100">
                      <SortHeader label="Nome da Cultura" ativo={sortKey === "nome"} dir={sortDir} onClick={() => toggleSort("nome")} className="max-w-[280px]" />
                      <SortHeader label="Tipo de Cultura" ativo={sortKey === "tipo"} dir={sortDir} onClick={() => toggleSort("tipo")} className="max-w-[180px]" />
                  ...
                  <tbody>
                    {pagina.map((c) => (
                      <tr key={c.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60 transition">
                        <td className="px-4 py-3 text-gray-500 text-sm whitespace-normal">{c.nome}</td>
                        <td className="px-4 py-3 text-gray-500 text-sm whitespace-normal">{c.tipo}</td>
                        <td className="px-4 py-3 text-gray-500 text-sm whitespace-normal">{c.situacao}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1 justify-end">
                            <button onClick={() => onNavigate("visualizar-cultura", c)} className="p-2 rounded-md hover:bg-green-50 transition" style={{ color: GREEN }} title="Visualizar"><ViewIcon size={18} /></button>
                            <button onClick={() => onNavigate("editar-cultura", c)} className="p-2 rounded-md hover:bg-green-50 transition" style={{ color: GREEN }} title="Editar"><Pencil size={17} /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
