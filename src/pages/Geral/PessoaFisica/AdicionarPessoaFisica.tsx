...
export function AdicionarPessoaFisicaPage({ onLogout, onNavigate }: { onLogout: () => void; onNavigate: (screen: any) => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#f2f3f5]">
      ...
      <main className="max-w-5xl mx-auto px-4 md:px-6 py-6 flex flex-col gap-5">
        <div>
          <button type="button" onClick={() => onNavigate("pessoa-fisica")} className="flex items-center gap-1 text-sm mb-3 text-[#1A7A3C] hover:opacity-70">
            <ArrowLeft size={15} /> Todas Pessoas Físicas
          </button>

          {/* Container que joga o botão verde para a direita */}
          <div className="flex justify-between items-center w-full">
            <h1 className="text-2xl font-semibold text-gray-900">Adicionar Pessoa Física</h1>
        ...
        <Section title="Informações Básicas">
          <div className="flex flex-col gap-3">
            <div className="flex gap-3">
              <FloatInput label="CPF" required value={cpf} onChange={handleCpfChange} maxLength={14} className="w-[200px] flex-shrink-0" />
            ...
            <div className="flex gap-3">
              <FloatSelect label="Sexo" required value={sexo} onChange={setSexo} options={[{ value: "M", label: "Masculino" }, { value: "F", label: "Feminino" }, { value: "O", label: "Outro" }, { value: "N", label: "Não informado" }]} className="flex-1" />
              <FloatSelect label="Estado Civil" required value={estadoCivil} onChange={setEstadoCivil} options={[{ value: "solteiro", label: "Solteiro(a)" }, { value: "casado", label: "Casado(a)" }, { value: "divorciado", label: "Divorciado(a)" }, { value: "viuvo", label: "Viúvo(a)" }]} className="flex-1" />
            </div>
          </div>
        </Section>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 animate-fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 flex flex-col items-center text-center gap-5 relative">
              ...
              <div className="flex flex-col gap-1 w-full">
                <h3 className="text-lg font-bold text-gray-900">Pessoa Física Adicionada com Sucesso!</h3>
