...
export function EditarPessoaFisica({ dadosIniciais, onLogout, onNavigate }: { dadosIniciais: any; onLogout: () => void; onNavigate: (screen: string, params?: any) => void }) {
...
const [proximoEstadoAtivo, setProximoEstadoAtivo] = useState(isCadastroAtivo);
  ...
  const handleSalvarEdicao = () => {
    const dadosAtualizados = {
      ...dadosIniciais,
      cpf, nome, apelido, dataNascimento, sexo, estadoCivil,
  ...
  return (
    <div className="min-h-screen bg-[#f2f3f5]">
      ...
      <main className="max-w-5xl mx-auto px-4 md:px-6 py-6 flex flex-col gap-5">
        <div>
          <button type="button" onClick={() => onNavigate("visualizar-pessoa-fisica", dadosIniciais)} className="flex items-center gap-1 text-sm mb-3 text-[#1A7A3C] hover:opacity-70">
            <ArrowLeft size={15} /> Visualizar Pessoa Física
          </button>
        ...
        <Section title="Informações Básicas">
          <div className="flex flex-col gap-3">
            <div className="flex gap-3">
              <FloatInput label="CPF" required value={cpf} onChange={(v) => setCpf(aplicarMascaraCPF(v))} maxLength={14} className="w-[200px] flex-shrink-0" />
              <FloatInput label="Nome" required value={nome} onChange={setNome} className="flex-1" />
            </div>
            <div className="flex gap-3">
              <FloatInput label="Apelido" value={apelido} onChange={setApelido} className="flex-1" />
              <FloatInput label="Data de Nascimento" type="date" required value={dataNascimento} onChange={setDataNascimento} className="w-[220px] flex-shrink-0" icon={<Calendar size={18} />} />          
            </div>
            <div className="flex gap-3">
              <FloatSelect label="Sexo" required value={sexo} onChange={setSexo} options={[{ value: "M", label: "Masculino" }, { value: "F", label: "Feminino" }, { value: "O", label: "Outro" }]} className="flex-1" />
              <FloatSelect label="Estado Civil" required value={estadoCivil} onChange={setEstadoCivil} options={[{ value: "solteiro", label: "Solteiro(a)" }, { value: "casado", label: "Casado(a)" }, { value: "divorciado", label: "Divorciado(a)" }]} className="flex-1" />
            </div>
          </div>
        </Section>
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8 flex flex-col items-center text-center gap-6 relative">
          ...
          <div className="flex flex-col gap-2 w-full mt-2">
            <h2 className="text-xl font-bold text-gray-800 tracking-tight">
              {proximoEstadoAtivo ? "Ativar Pessoa Física" : "Inativar Pessoa Física"}
            </h2>
            <p className="text-sm text-gray-600 font-normal leading-relaxed mt-1 px-1">
              Deseja {proximoEstadoAtivo ? "ativar" : "inativar"} o cadastro da Pessoa Física{" "}
              <span className="font-semibold text-gray-800">
                { nome ? nome : "Divino de Souza Sobrinho"}
              </span>?
            </p>
          </div>
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 animate-fade-in">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 flex flex-col items-center text-center gap-5 relative">
      ...
      <div className="flex flex-col gap-1 w-full">
        <h3 className="text-lg font-bold text-gray-900">Salvar Alterações da Pessoa Física!</h3>
        <p className="text-sm text-gray-500 mt-1 px-2">
          Deseja salvar as alterações da Pessoa Física <span className="font-semibold text-gray-700">
            {nome ? nome : "A nova pessoa física"}
          </span>?
        </p>
      </div>
      ...
      <div className="flex gap-3 w-full mt-1">
        ...
        <button
          ...
          onClick={() => {
            ...
            onNavigate("pessoa-fisica"); // Redireciona para a listagem geral
          }}
        ...
        <button
          ...
          onClick={() => {
            ...
            onNavigate("visualizar-pessoa-fisica", dadosSalvosTemporarios);
          }}
