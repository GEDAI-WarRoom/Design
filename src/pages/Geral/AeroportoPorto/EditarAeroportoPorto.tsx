...
import { BlocoEnderecoFields, BlocoContatoFields } from "../../../components/ui/EntitySearch";
...
export function EditarAeroportoPortoPage({ dados, onLogout, onNavigate }: { dados?: any; onLogout: () => void; onNavigate: (screen: string, data?: any) => void; }) {
  ...
  const [endereco, setEndereco] = useState({
    zona: dados?.zona || "Urbana", cep: dados?.cep || "", estado: dados?.estado || "Minas Gerais", municipio: dados?.municipio || "", bairro: dados?.bairro || "",
    endereco: dados?.endereco || "", numero: dados?.numero || "", complemento: dados?.complemento || "", localidade: "", distrito: "", latitude: "", longitude: ""
  });
  
  const [contatos, setContatos] = useState({
    utilizarContatoProprietario: "Não" as const, proprietariosSelecionados: [] as string[],
    emailFixo: "contato@local.com", emailFixoObs: "", telefoneFixo: "(35) 99999-9999", telefoneFixoObs: "", contatosAdicionais: [] as any[]
  });
      ...
      <main className="max-w-[1088px] mx-auto px-4 md:px-6 py-6 flex flex-col gap-4">
        ...
        <Section title="Informações Básicas">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <FloatInput
              label="Nome" required value={nome} onChange={setNome} maxLength={255}
            />
            <FloatSelect
              label="Tipo" required value={tipo} onChange={setTipo}
              options={[ { value: "Aeroporto", label: "Aeroporto" }, { value: "Porto", label: "Porto" }, { value: "Posto de Fronteira", label: "Posto de Fronteira" } ]}
            />
            <FloatSelect
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 text-center animate-fadeIn">
            ...
            <div className="flex gap-3 justify-center mt-6">
              ...
              <button onClick={() => { setIsSucesso(false); onNavigate("visualizar-aeroporto-porto", dados); }} className="px-5 h-11 rounded-md bg-[#1A7A3C] hover:bg-[#15612F] text-white text-sm font-semibold transition">Visualizar</button>
            </div>
          </div>
        </div>
      )}
