      ...
      <main className="max-w-[1088px] mx-auto px-4 md:px-6 py-6 flex flex-col gap-4">
        ...
        <Section title="Informações Básicas">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <FloatInput label="Tipo do Veículo" value={veiculo.tipo || "-"} disabled onChange={() => {}} />
            <FloatInput label="Situação" value={veiculo.situacao || "Ativo"} disabled onChange={() => {}} />
          </div>
        </Section>

        <Section title="Observações">
          <LargeTextArea label="Observações" value={veiculo.observacao || "Nenhuma observação registrada."} disabled onChange={() => {}} />
        </Section>
      </main>
    </div>
  );
