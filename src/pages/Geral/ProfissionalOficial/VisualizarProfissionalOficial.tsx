...
export function VisualizarProfissionalOficialPage({ dados, onLogout, onNavigate }: { dados?: any; onLogout: () => void; onNavigate: (s: string, d?: any) => void; }) {
      ...
      <main className="max-w-[1088px] mx-auto px-4 md:px-6 py-6 flex flex-col gap-4">
        ...
        <Section title="Informações Básicas">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <FloatInput label="Matrícula / MASP" value={profissional.matricula || "-"} disabled onChange={() => {}} />
            <FloatInput label="Situação" value={profissional.situacao || "Ativo"} disabled onChange={() => {}} />
          </div>
        </Section>

        <Section title="Observações">
          <LargeTextArea label="Observações" value={profissional.observacao || "Nenhuma observação registrada."} disabled onChange={() => {}} />
        </Section>
