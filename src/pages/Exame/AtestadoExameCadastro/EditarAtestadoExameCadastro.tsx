...
export function EditarAtestadoExameCadastroPage({
  dados,
  onLogout,
  onNavigate,
}: {
  ...
  const [value, setValue] = useState<AtestadoExameCadastro>(() => dados ?? listarAtestadosCadastro()[0]);
  ...
  return (
    <div className="min-h-screen bg-[#f2f3f5] pb-16">
      ...
      <main className="mx-auto flex max-w-[1088px] flex-col gap-4 px-4 py-6 md:px-6">
        <div>
          <button type="button" onClick={() => onNavigate("visualizar-cadastro-atestado-exame", value)} className="mb-3 flex items-center gap-1 text-sm font-semibold text-[#1A7A3C]">
            <ArrowLeft size={15} /> Visualizar Atestado de Exame
          </button>
        ...
        {tentouSalvar && !atestadoCadastroValido(value) && (
          <div className="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">
            <AlertTriangle size={18} /> Corrija os campos indicados antes de salvar.
          </div>
        )}
        <AtestadoExameCadastroForm value={value} onChange={setValue} mode="edit" showErrors={tentouSalvar} />
      </main>
      ...
      {salvo && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-8 text-center shadow-xl">
            ...
            <h2 className="text-xl font-bold text-gray-900">Atestado atualizado com sucesso!</h2>
