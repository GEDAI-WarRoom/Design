...
const TIPOS_DESTINATARIO = [
  { value: "Revendedora de Produtos Agropecuários", label: "Revendedora de Produtos Agropecuários" },
...
export function VendaComEntradaInsumosExamesDetalhe({ dados, mode, onSalvar }: Props) {
  ...
  const totais = useMemo(() => form.lotes.reduce((acc: Record<string, number>, lote: any) => { const nome = lote.doenca?.nome || "Doença não informada"; const total = lote.apresentacoes.reduce((soma: number, ap: any) => soma + (Number(ap.dosesPorFrasco) || 0) * (Number(ap.frascos) || 0), 0); acc[nome] = (acc[nome] || 0) + total; return acc; }, {}), [form.lotes]);
  ...
  return <>
    ...
    <Section title="Destinatário"><div className="flex flex-col gap-4">
      <FloatSelect label="Tipo de Destinatário" required value={form.destinatario} disabled={disabled} onChange={(destinatario) => setForm({ ...form, destinatario })} options={TIPOS_DESTINATARIO} />
      {form.destinatario === "Médico Veterinário" ? <FloatInput label="Médico Veterinário" required value={form.medicoVeterinario?.nome || "Dr. José Firmino"} disabled={disabled} onChange={(nome) => setForm({ ...form, medicoVeterinario: { ...form.medicoVeterinario, nome } })} /> : <FloatInput label="Revendedora" required value={form.revendedora?.nome || ""} disabled={disabled} onChange={(nome) => setForm({ ...form, revendedora: { ...form.revendedora, nome } })} />}
    </div></Section>

    <Section title="Nota Fiscal"><div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><FloatInput label="Número da Nota Fiscal" required value={form.numeroNotaFiscal} disabled={disabled} onChange={(numeroNotaFiscal) => setForm({ ...form, numeroNotaFiscal: numeroNotaFiscal.replace(/\D/g, "").slice(0, 10) })} /><FloatSelect label="UF da Nota Fiscal" required value={form.ufNotaFiscal} disabled={disabled} onChange={(ufNotaFiscal) => setForm({ ...form, ufNotaFiscal })} options={UFS} /></div>
      ...
      {form.lotes.map((lote: any, loteIndex: number) => {
        ...
        return <article key={lote.uid || loteIndex} className="border-l-4 border-l-[#1A7A3C] rounded-r-xl rounded-l-md bg-gray-50/40 border border-gray-100 p-5 flex flex-col gap-5 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4"><FloatInput label="Doença" value={lote.doenca?.nome || ""} disabled={disabled} onChange={(nome) => alterarLote(loteIndex, { doenca: { ...lote.doenca, nome } })} /><FloatInput label="Tipo de Reagente" value={lote.tipoInsumoExame || ""} disabled={disabled} onChange={(tipoInsumoExame) => alterarLote(loteIndex, { tipoInsumoExame })} /><FloatInput label="Data de validade" required value={lote.validade || ""} disabled={disabled} icon={<Calendar size={18} />} onChange={(validade) => alterarLote(loteIndex, { validade })} /></div>
      ...
      <div className="border-t border-gray-100 pt-5 flex flex-col gap-3"><span className="text-sm font-semibold text-gray-700">Total da Nota</span>{Object.entries(totais).map(([doenca, total]) => <div key={doenca} className="grid grid-cols-1 md:grid-cols-2 gap-4"><FloatInput label="Doença" value={doenca} disabled onChange={() => {}} /><FloatInput label="Total de Doses Adquiridas" value={String(total)} disabled onChange={() => {}} /></div>)}</div>
    </div></Section>
