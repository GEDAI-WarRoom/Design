...
export function PendenciasResumo({
	...
	const quantidadePendencias = totalCount ?? items.length;
			...
			{visibleItems.length > 0 ? (
				<div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-3">
					{visibleItems.map((item) => (
						<article key={item.id} className="flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition duration-200 ease-out hover:-translate-y-1 hover:shadow-lg">
							<div className="flex flex-1 flex-col p-4">
								...
								{item.details?.length ? (
									<div className="mt-4 space-y-2 text-xs text-gray-500">
										{item.details.map((detail) => <p key={detail} className="truncate">{detail}</p>)}
									</div>
								) : null}
							</div>
							{item.onAction && (
